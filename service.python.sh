...
# step 2: modify the SERVICE_NAME, PATH_TO_PY, and choose a PID_PATH_NAME 
...
PATH_TO_PY=/usr/local/codeautodeploy/codeautodeploy.py
...
case $1 in
    start)
        ...
        if [ ! -f $PID_PATH_NAME ]; then
            python $PATH_TO_PY /tmp 2>> /dev/null >> /dev/null &
                        echo $! > $PID_PATH_NAME
